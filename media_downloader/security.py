    ...
    for item in os.environ.get("MEDIA_DOWNLOADER_ALLOW_PRIVATE_HOSTS", "").split(","):
        ...
        if any(char in host for char in "*/:@"):
            raise MediaSecurityError(
                "Private-host allowlist entries must be exact hosts"
            )
        hosts.add(host)
    return hosts
...
def validate_media_url(url: str) -> str:
    ...
    for address in addresses:
        try:
            ...
            raise MediaSecurityError(
                "Media host resolved to an invalid address"
            ) from exc
    ...
    for _ in range(_MAX_REDIRECTS + 1):
        ...
        for chunk in response.iter_content(chunk_size=64 * 1024):
            ...
            if len(payload) > _MAX_METADATA_BYTES:
                response.close()
                raise MediaSecurityError(
                    "Media metadata response exceeded its size limit"
                )
...
def resolve_output_directory(
    ...
    try:
        ...
        raise MediaSecurityError(
            "Download directory is outside the media output root"
        ) from exc
    ...
    try:
        ...
        raise MediaSecurityError(
            "Download directory escaped through a symbolic link"
        ) from exc
...
def contained_output_path(path: str, root: Path) -> Path:
    ...
    try:
        ...
        raise MediaSecurityError(
            "Downloader produced a path outside its output root"
        ) from exc
    return candidate
